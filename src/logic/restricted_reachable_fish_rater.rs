...
use super::Rater;
use super::penguin_restrictions::PenguinRestrictions;

pub struct RestrictedReachableFishRater;

impl RestrictedReachableFishRater {
    fn get_reachable_fish_from_penguin(penguin: Penguin, penguin_restrictions: PenguinRestrictions, board: &Board, checked_field_bitset: &mut Bitset8x8) -> i32 {
    ...
    fn reachable_fish_count_of_team(game_state: &State, team: Team) -> i32 {
        ...
        for penguin in penguins {
            let penguin_restrictions = PenguinRestrictions::for_penguin(&penguin, &game_state.board);
            let fish_count = Self::get_reachable_fish_from_penguin(penguin, penguin_restrictions, &game_state.board, &mut checked_field_bitset);
...
mod tests {
    ...
    fn given_empty_board_with_team_one_penguin_on_one_tile_island_then_reachable_fish_count_of_team_one_is_zero() {
        ...
        let fish_count = RestrictedReachableFishRater::reachable_fish_count_of_team(&state, Team::One);
    ...
    #[test]
    fn given_board_with_restricted_part_then_reachable_fish_count_is_correct() {
        let mut board = Board::empty();
    ...
    fn given_board_with_penguins_of_both_teams_and_bigger_fish_field_island_then_rating_is_correct() {
        ...
        board.set(Coordinate::new(11, 5), FieldState::Fish(3)).unwrap();
        let state = State::from_initial_board_with_start_team_one(board);
        let actual = RestrictedReachableFishRater::rate(&state);
