...
impl std::fmt::Display for ProtocolError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ProtocolError::InvalidJoinRoomId { room_id } => {
                write!(f, "Server couldn't find a room with id {room_id}")
            }
            ProtocolError::InvalidReservation { reservation } => write!(
                f,
                "Server couldn't find a reservation for the code {reservation}"
            ),
        ...
        if let Some(original_request) = packet.original_request {
            match original_request.class.as_str() {
                "joinRoom" if original_request.room_id.is_some() => {
                    return Self::InvalidJoinRoomId {
                        room_id: original_request.room_id.unwrap(),
                    }
                }
                "joinPrepared" if original_request.reservation_code.is_some() => {
                    return Self::InvalidReservation {
                        reservation: original_request.reservation_code.unwrap(),
                    }
                }
                _ => {}
            }
...
mod tests {
    ...
    fn invalid_join_room_id_error_from_errorpacket() {
        ...
        let expected = ProtocolError::InvalidJoinRoomId {
            room_id: "amogus".to_string(),
        };
    ...
    fn invalid_reservation_code_error_from_errorpacket() {
        ...
        let expected = ProtocolError::InvalidReservation {
            reservation: "amogus".to_string(),
        };
        let actual = ProtocolError::from(packet);
        assert_eq!(expected, actual);
    }
    ...
    fn other_error_from_errorpacket() {
        ...
        let expected = ProtocolError::Other {
            message: "sc.server.gaming.OtherException: This is some other error.".to_string(),
        };
        let actual = ProtocolError::from(packet);
        assert_eq!(expected, actual);
