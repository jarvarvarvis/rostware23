...
impl PossibleMovesIterator {
    ...
    pub fn make_normal_moves_iterator_for_team(state: State, team: Team) -> Self {
        let penguin_iterator = state
            .board
            .get_penguin_iterator(team)
            .flat_map(move |penguin| {
                PenguinPossibleMoveIterator::from(penguin, state.board.clone())
            });
    ...
    fn make_beginning_place_moves_iterator(state: State) -> Self {
        ...
        let moves = board_coordinate_iterator
            .filter(
                move |coordinate| match state.board.get(coordinate.clone()) {
...
mod tests {
    ...
    fn possible_moves_iterator_gives_correct_moves_on_simple_board() {
        ...
        let mut team_one_possible_moves_iter =
            PossibleMovesIterator::from_state_and_team(state.clone(), Team::One);
        assert_eq!(
            Move::Normal {
                from: Coordinate::new(2, 0),
                to: Coordinate::new(0, 0)
            },
            team_one_possible_moves_iter.next().unwrap()
        );
        assert_eq!(
            Move::Normal {
                from: Coordinate::new(6, 0),
        ...
        let mut team_two_possible_moves_iter =
            PossibleMovesIterator::from_state_and_team(state, Team::Two);
