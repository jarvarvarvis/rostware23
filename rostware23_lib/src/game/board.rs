...
impl Board {
    ...
    pub fn fill(field_state: FieldState) -> Self {
        ...
        for y in 0..BOARD_HEIGHT {
            for x in 0..BOARD_WIDTH {
                board
                    .set(
                        Coordinate::new(x, y).odd_r_to_doubled(),
                        field_state.clone(),
                    )
                    .unwrap();
            }
    ...
    pub fn get(&self, at: Coordinate) -> anyhow::Result<FieldState> {
        ...
        if is_fish_field {
            let greater_than_two = self
                .if_fish_field_then_fish_count_higher_than_two_otherwise_penguin_team
                .get(x.into(), y.into())?;
            let odd = self
                .if_fish_field_then_fish_modulo_2_otherwise_penguin_count
                .get(x.into(), y.into())?;
        ...
        let is_penguin = self
            ...
            .get(x.into(), y.into())?;
        if is_penguin {
            let penguin_in_team_two = self
                .if_fish_field_then_fish_count_higher_than_two_otherwise_penguin_team
                .get(x.into(), y.into())?;
            return Ok(FieldState::Team(if penguin_in_team_two {
                Team::Two
            } else {
                Team::One
            }));
    ...
    pub fn set(&mut self, at: Coordinate, field_state: FieldState) -> anyhow::Result<()> {
        ...
        self.if_fish_field_then_fish_count_higher_than_two_otherwise_penguin_team
            .set(
                x.into(),
                y.into(),
                get_fish_higher_than_two_or_penguin_team_for_field(&field_state),
            )?;
        self.if_fish_field_then_fish_modulo_2_otherwise_penguin_count
            .set(
                x.into(),
                y.into(),
                get_fish_modulo_2_equals_1_or_is_penguin(&field_state),
            )?;
    ...
    fn perform_normal_move(
        ...
        self.set(to.clone(), FieldState::Team(team.clone()))?;
        self.penguin_collection.move_penguin(
            Penguin {
                coordinate: from,
                team,
            },
...
mod tests {
    ...
    fn place_moves_update_correct_fields() {
        ...
        assert_eq!(
            board.get(Coordinate::new(2, 4)).unwrap(),
            FieldState::Team(Team::One)
        );
        assert_eq!(
            board.get(Coordinate::new(7, 1)).unwrap(),
            FieldState::Team(Team::Two)
        );

        assert_eq!(
            board.get(Coordinate::new(3, 5)).unwrap(),
            FieldState::Fish(2)
        );
    ...
    fn penguin_collection_iterator_on_updated_board_is_correct() {
        ...
        board
            .perform_move(Move::Place(coord_1.clone()), Team::One)
            .unwrap();
        let coord_2 = Coordinate::new(7, 1);
        board
            .perform_move(Move::Place(coord_2.clone()), Team::One)
            .unwrap();
        ...
        assert_eq!(
            Penguin {
                coordinate: coord_1.clone(),
                team: Team::One
            },
            penguin_iterator.next().unwrap()
        );
        assert_eq!(
            Penguin {
                coordinate: coord_2.clone(),
                team: Team::One
            },
            penguin_iterator.next().unwrap()
        );
    ...
    fn penguin_collection_iterator_on_board_with_all_penguins_is_correct() {
        ...
        assert_eq!(
            Penguin {
                coordinate: Coordinate::new(12, 0),
                team: Team::One
            },
            team_one_penguins.next().unwrap()
        );
    ...
    fn can_move_to_non_player_field() {
        let mut board = Board::fill(FieldState::Fish(2));
        board
            .perform_move(Move::Place(Coordinate::new(0, 0)), Team::One)
            .unwrap();
    ...
    fn can_not_move_to_player_field() {
        ...
        board
            ...
            .unwrap();
        board
            .perform_move(Move::Place(Coordinate::new(1, 3)), Team::Two)
            .unwrap();
        assert!(!board.can_move_to(Coordinate::new(1, 3)).unwrap());
    }
}
